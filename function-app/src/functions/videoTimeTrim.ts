...
export async function videoTimeTrim(blob: Buffer, context: InvocationContext): Promise<void> {
    const blobName = (context.triggerMetadata?.name as string) ?? `video-${randomUUID()}`;

    context.log('videoTimeTrim triggered', { blobName, size: blob.length });
    ...
    try {
        ...
        context.log('videoTimeTrim wrote blob to temp file', { tmpInputPath });
        ...
        context.log('videoTimeTrim starting pipeline', { blobName, options });
        ...
        context.log('videoTimeTrim pipeline succeeded', {
        ...
        if (error instanceof NoSegmentsDetectedError) {
            context.log('videoTimeTrim no segments detected', { blobName });
            return;
        }
        context.log('videoTimeTrim function error', error);
        throw error;
    } finally {
        if (fs.existsSync(tmpInputPath)) {
            try {
                ...
                context.log('videoTimeTrim failed to clean up temp file', cleanupErr);
            }
        }
    }
}

app.storageBlob('videoTimeTrim', {
